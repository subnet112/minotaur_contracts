...
Thanks for your interest in the Minotaur platform contracts. This guide covers the basics — how to file an issue, how to run the tests locally, and what to expect from the review process.
...
3. Run the test suite locally before pushing:
   ...
   ```bash
   git submodule update --init    # one-time: fetch OpenZeppelin et al.
   forge build                    # compile
   forge test -v                  # full test suite
   ```
